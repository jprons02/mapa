...
class MediaList extends React.Component {
  ...
  sortItems = () => {
    ...
    const mediaType = (languageList) => {
      const videoRegex = /((.mp4$)|(.wmv$)|(.mov$))/gim;
      const audioRegex = /((.mp3$)|(.m4a$)|(.wav$))/gim;
      const imageRegex = /((.png$)|(.jpg$))/gim;
      const logoRegex = /logo/gim;
      const zipRegex = /\.zip$/gim;
      const videoList = [];
      const audioList = [];
      const imageList = [];
      const logoList = [];
                ...
                <List.Item style={{ marginBottom: "4px" }} key={listItem.id}>
                  ...
                  <List.Content>
                    <List.Item
                      onClick={() =>
                        this.downloadFile(
                          `/api/download${listItem.path_lower}`,
                          listItem.name
                        )
                      }
                      as="a"
      ...
      <React.Fragment>
        ...
        <Segment style={{ padding: "20px 14px 40px 14px" }} raised>
          <div
            ...
            style={{
              marginBottom: "30px",
              display: this.props.isDownloading ? "block" : "none" || "none",
            }}
          >
          ...
          <div>
            ...
            {this.props.mediaList.entries
              ? this.renderList("english")
              : "loading list..."}
          </div>
          <div style={{ marginTop: "60px" }}>
            <Header as="h2">Spanish</Header>
            {this.props.mediaList.entries
              ? this.renderList("spanish")
              : "loading list..."}
          </div>
        </Segment>
      </React.Fragment>
...
export default connect(mapStateToProps, { fetchList, downloadingFile })(
  MediaList
);
